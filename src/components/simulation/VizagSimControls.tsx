...
interface VizagSimControlsProps {
  cars: VizagSimCar[];
  isRunning: boolean;
...
export const VizagSimControls: React.FC<VizagSimControlsProps> = ({
  cars,
  isRunning,
  proximityThreshold,
  activeAlerts,
    ...
    <div className="space-y-5">
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-5 shadow-xl border border-slate-800">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-black tracking-tight text-white uppercase">
                Simulation Controls & Safety Engine
              </h2>
              {activeAlerts.length > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-red-500/30 text-red-300 border border-red-500/40 text-[10px] font-black animate-pulse">
                  ⚠️ {activeAlerts.length} Alert Active
                </span>
              )}
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              4 autonomous vehicles navigating Visakhapatnam's connected road graph.
            </p>
          </div>
        ...
        <div className="mt-4 pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 w-full sm:w-64">
            <span className="text-[10px] text-slate-500 font-mono">20m</span>
            <input
              type="range"
              min={20}
              max={100}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {cars.map((car) => {
          ...
          const availableTurns = vizagSimEngine.getAvailableTurnsForCar(car.id);
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 space-y-1.5">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-bold text-slate-700 flex items-center gap-1 truncate">
                    ...
                    Next: <span className="text-slate-900 font-black">{car.upcomingNodeName}</span>
                  </span>
                  ...
                  {availableTurns.slice(0, 4).map((turn) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-[10px] font-bold px-2 py-1 rounded-lg border transition truncate max-w-[140px] ${
                          isTurnSelected
      ...
      {/* ── 3. Real-time Proximity Matrix & Distance Monitor ── */}
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Activity className="w-4 h-4 text-indigo-600" />
            <h3 className="text-xs font-black uppercase text-slate-800 tracking-wider">
              Real-Time Inter-Car Proximity Distance Matrix
            </h3>
          </div>
